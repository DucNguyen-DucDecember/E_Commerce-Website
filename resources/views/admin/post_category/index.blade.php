    ...
    <div class="row">
        <div class="col-4">
            <div class="card">
                ...
                @endif
                <div class="card-body">
                    <form action="{{url('admin/post_category/add')}}" method="post">
                        @csrf
                        ...
                        <tbody>
                            ...
                            @foreach ($post_cats as $item)
                                <tr>
                                    ...
                                    <td><a href="{{route('post_category_edit', $item->id)}}"
                                            class="btn btn-success btn-sm rounded-0" type="button" data-toggle="tooltip"
                                            data-placement="top" title="Edit"><i class="fa fa-edit"></i></a>
                                        <a href="javascript:void(0)"
                                            class="btn btn-danger btn-sm rounded-0 delete-post-category" type="button" data-id="{{$item->id}}"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        document.querySelectorAll('.delete-post-category').forEach(function (button) {
            button.addEventListener('click', function () {
                var itemId = this.getAttribute('data-id');
                var deleteUrl = `{{ route('post_category_delete', ':id') }}`.replace(':id', itemId);
